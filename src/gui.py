...
class CrackerGUI:
    ...
    def _build_ui(self):
        self.body = ctk.CTkScrollableFrame(self.root, fg_color="transparent")
        ...
        # Input card
        inp = ctk.CTkFrame(self.body, corner_radius=12)
        ...
        # Generate panel
        gen = ctk.CTkFrame(self.body, corner_radius=12)
        ...
        # Control bar
        ctl = ctk.CTkFrame(self.body, corner_radius=12)
        ...
        ctk.CTkLabel(prog, text="Live Processing", font=ctk.CTkFont(size=14, weight="bold")).grid(
            row=0, column=0, sticky="w", padx=14, pady=(10, 4))

        self.bars = {}
        self.count_lbls = {}
        self.word_lbls = {}
        for idx, mode in enumerate(("Sequential", "Parallel"), start=1):
            color = SEQ_COLOR if mode == "Sequential" else PAR_COLOR
            section = ctk.CTkFrame(prog, fg_color=BG_DARK, corner_radius=8)
            section.grid(row=idx, column=0, sticky="ew", padx=12, pady=4)
            section.grid_columnconfigure(1, weight=1)

            ctk.CTkLabel(section, text=mode, width=84,
                         font=ctk.CTkFont(size=12, weight="bold"),
                         text_color=color).grid(row=0, column=0, rowspan=2, padx=10, pady=8)

            pb = ctk.CTkProgressBar(section, mode="determinate", progress_color=color)
            pb.set(0)
            pb.grid(row=0, column=1, sticky="ew", padx=8, pady=(8, 2))
            self.bars[mode] = pb

            count = ctk.CTkLabel(section, text="0 / 0  (0%)",
                                 font=ctk.CTkFont(size=11),
                                 text_color=TEXT_DIM, width=140, anchor="e")
            count.grid(row=0, column=2, padx=10, pady=(8, 2))
            self.count_lbls[mode] = count

            word = ctk.CTkLabel(section, text="—",
                                font=ctk.CTkFont(family="Consolas", size=11),
                                text_color="white", anchor="w")
            word.grid(row=1, column=1, columnspan=2, sticky="ew", padx=10, pady=(0, 8))
            self.word_lbls[mode] = word

        ctk.CTkLabel(prog, text="").grid(row=3, column=0, pady=2)
        ...
        right = ctk.CTkFrame(self.body, corner_radius=12)
    ...
    def _embed_charts(self, parent):
        self.fig_time = Figure(figsize=(3.2, 2.6), dpi=100, facecolor=BG_CARD)
        self.ax_time = self.fig_time.add_subplot(111)
        self._style_axes(self.ax_time, "Time Taken (s)")
        self.canvas_time = FigureCanvasTkAgg(self.fig_time, master=parent)
        self.canvas_time.get_tk_widget().grid(row=1, column=0, sticky="nsew", padx=(10, 5), pady=(4, 10))

        self.fig_wps = Figure(figsize=(3.2, 2.6), dpi=100, facecolor=BG_CARD)
        ...
        self.canvas_wps.get_tk_widget().grid(row=1, column=1, sticky="nsew", padx=(5, 10), pady=(4, 10))
    ...
    def _refresh_charts(self):
        ...
        self.ax_time.set_ylim(bottom=0, top=max(times) * 1.2 if max(times) > 0 else 1)
        ...
        for b, w in zip(bars, wps):
            if w > 0:
                self.ax_wps.text(b.get_x() + b.get_width() / 2, b.get_height(),
                                 ...
                                 color="white", fontsize=9, fontweight="bold")
        self.ax_wps.set_ylim(bottom=0, top=max(wps) * 1.2 if max(wps) > 0 else 1)
    ...
    def _clear_state(self):
        self.results.clear()
        for col in ("seq", "par"):
            for row, lbl in self._cells[col].items():
                lbl.configure(text="—", text_color="white")
        for mode in ("Sequential", "Parallel"):
            self.bars[mode].set(0)
            self.count_lbls[mode].configure(text="0 / 0  (0%)")
            self.word_lbls[mode].configure(text="—")
    ...
    def _reset_progress(self, mode: str):
        def reset():
            self.bars[mode].set(0)
            self.count_lbls[mode].configure(text="0 / 0  (0%)")
            self.word_lbls[mode].configure(text="starting...")
        self.root.after(0, reset)

    def _update_progress(self, mode: str, checked: int, total: int, word):
        frac = (checked / total) if total else 0
        frac = min(max(frac, 0.0), 1.0)
        text_count = f"{checked:,} / {total:,}  ({frac * 100:.1f}%)"
        text_word = f"› {word}" if word else "—"

        def apply():
            self.bars[mode].set(frac)
            self.count_lbls[mode].configure(text=text_count)
            self.word_lbls[mode].configure(text=text_word)
        self.root.after(0, apply)

    def _finish_progress(self, mode: str):
        def done():
